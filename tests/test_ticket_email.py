...
async def _seed_data(session):
    cinema = Cinema(
        id=uuid.uuid4(), name="SHENOYS Lulu Mall", city="Kochi", timezone="Asia/Kolkata"
    )
...
async def test_t19_email_sent_and_public_ticket_link(session, session_factory):
    test_email_service = ConsoleEmailService(web_base_url="http://shenoys.demo")
    ...
    user = User(id=uuid.uuid4(), email="ticket_fan@shenoys.local", password_hash="h")
    session.add(user)
    await session.commit()
    ...
    async with AsyncClient(
        ...
        assert sent["to"] == "ticket_fan@shenoys.local"

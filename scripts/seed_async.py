...
CINEMAS = [{'name': 'Shenoys', 'screens': 1}]
CITY = 'Kochi'
# Shenoys: classic single-screen with one raked row of 200 seats
ROWS = [('A', 200, 25000)]
MOVIES = [
...
async def main():
    ...
    async with S() as s:
        ...
        u = (await s.execute(select(User).where(User.email=='demo@shenoys.local'))).scalar_one_or_none()
        if not u:
            u = User(id=uuid.uuid4(), email='demo@shenoys.local', password_hash=pwd.hash('demo1234'), role='PARTNER')
